...
```mermaid
flowchart LR
    SRC-001["Writable C:\ProgramData\Bitdefender Agent\bdch\ allows DLL p<br/>F-006 · ac=y · ⏳ unexplored"]:::grpF
    BIN([productagent.dll<br/>SYSTEM]):::binary
...
flowchart TB
    subgraph ZSYSTEM["SYSTEM"]
        Nproductagent_dll["**productagent.dll**<br/>(this binary)"]:::self
    end
...
_41 relevant classes; 1 present · 0 defense observed · 0 not present · 40 unchecked_
...
| `F-006` World-writable / user-readable config / log file | 🔴 present | sources: `SRC-001`; chains: `CHAIN-001` |
...
|----|------|-----|------|---------------------|------------|-------|
| SRC-001 | Writable C:\ProgramData\Bitdefender Agent\bdch\ allows DLL plant; SYSTEM service may LoadLibrary | (see finding) | (unspecified) | yes | 27.x | Auto-enriched from bitdefender-total-security-2026-04-11/001-bdch-dll-hijack.md. World-writable config-file/dir variant per F-006; could also map to UP-001 if treated as code-load. |
...
|----|-------|---------------|--------|----------|
| [CHAIN-001](#chain-001) | Writable C:\ProgramData\Bitdefender Agent\bdch\ allows DLL plant; SYSTEM service | `Writable C:\ProgramData\Bitdefender Agent\bdch\ allows DLL plant; SYSTEM service may LoadLibrary` → `Elevated file operation (write / set_acl / read)` | unexplored | — |

### CHAIN-001 — Writable C:\ProgramData\Bitdefender Agent\bdch\ allows DLL plant; SYSTEM service

**Status:** ⏳ unexplored  
**CWE:** CWE-269  
**Finding:** [`bitdefender-total-security-2026-04-11/findings/001-bdch-dll-hijack.md`](../../engagements/bitdefender-total-security-2026-04-11/findings/001-bdch-dll-hijack.md)  

| Source | Conditions | Sink | Impact |
|--------|------------|------|--------|
| `Writable C:\ProgramData\Bitdefender Agent\bdch\ allows DLL plant; SYSTEM service may LoadLibrary` | (auto-stub: walk the source through any defenses, list each check that must pass) | `Elevated file operation (write / set_acl / read)` | arbitrary file access at privileged principal's level |

```mermaid
flowchart LR
    SRC["Writable C:\ProgramData\Bitdefender Agent\bdch\ allows DLL plant; SYSTEM service may LoadLibrary"]:::src
...
---
_Auto-generated by `scripts/catalog_render.py` at 2026-05-09 15:29 UTC. Edit `catalog/binaries/productagent_dll.yml` then re-run the renderer._
