...
```mermaid
flowchart LR
    SRC-001["Server-supplied filename in restore API response flows to Mo<br/>N-003 · ac=y+caveat · ⏳ unexplored"]:::grpN
    BIN([bzrestore<br/>unknown]):::binary
    SNK-001["Network parser memory corruption / logic flaw<br/>CWE-787"]:::sink
    SRC-001 --> BIN
    BIN --> SNK-001
...
flowchart TB
    ...
    subgraph Zunknown["unknown"]
        Nbzrestore["**bzrestore**<br/>(this binary)"]:::self
    end
...
_62 relevant classes; 1 present · 0 defense observed · 0 not present · 61 unchecked_
...
| `N-003` TLS / network protocol parser (transport-level) | 🔴 present | sources: `SRC-001`; chains: `CHAIN-001` |
...
|----|------|-----|------|---------------------|------------|-------|
| SRC-001 | Server-supplied filename in restore API response flows to MoveFileExW without prefix validation | (see finding) | (unspecified) | yes (caveat: Source is the Backblaze server API; requires compromised/malicious server, MITM, or compromised account) | — | Auto-enriched from backblaze-2026-04-08/003-restore-path-traversal.md. Server-as-attacker model; closest fit is N-003 (TLS/network protocol parser) since it's protocol-layer rather than browser/HTTP. Could also be N-004 (custom application protocol). |
...
|----|------|-----|----------|--------|------------|
| SNK-001 | Network parser memory corruption / logic flaw | CWE-787 | (see finding evidence) | remote code execution or DoS | — |
...
|----|-------|---------------|--------|----------|
| [CHAIN-001](#chain-001) | Server-supplied filename in restore API response flows to MoveFileExW without pr | `Server-supplied filename in restore API response flows to MoveFileExW without prefix validation` → `Network parser memory corruption / logic flaw` | unexplored | — |

### CHAIN-001 — Server-supplied filename in restore API response flows to MoveFileExW without pr

**Status:** ⏳ unexplored  
**CWE:** CWE-787  
**Finding:** [`backblaze-2026-04-08/findings/003-restore-path-traversal.md`](../../engagements/backblaze-2026-04-08/findings/003-restore-path-traversal.md)  

| Source | Conditions | Sink | Impact |
|--------|------------|------|--------|
| `Server-supplied filename in restore API response flows to MoveFileExW without prefix validation` | (auto-stub: walk the source through any defenses, list each check that must pass) | `Network parser memory corruption / logic flaw` | remote code execution or DoS |

```mermaid
flowchart LR
    SRC["Server-supplied filename in restore API response flows to MoveFileExW without prefix validation"]:::src
...
---
_Auto-generated by `scripts/catalog_render.py` at 2026-05-09 15:29 UTC. Edit `catalog/binaries/bzrestore.yml` then re-run the renderer._
