...
```mermaid
flowchart LR
    SRC-001["Sophos hardens C:\ProgramData\Sophos with explicit non-inher<br/>F-001 · ac=n · ❔ hypothesised"]:::grpF
    BIN([sophossetup.exe<br/>installer-elevated]):::binary
...
flowchart TB
    ...
    subgraph Zinstaller_elevated["installer-elevated"]
        Nsophossetup_exe["**sophossetup.exe**<br/>(this binary)"]:::self
    end
...
|---------|------------|------------|--------|-------|
| — | 2026-04-29 | sophos-2026-04-29 | — | — |
| trial-2026-05-05-latest | 2026-05-05 | sophos-endpoint-ipc-2026-05-05 | — | Auto-seeded from sophos-endpoint-ipc-2026-05-05/scope.json (target: Sophos Intercept X Endpoint (Windows)) |
...
|----|------|-----|------|---------------------|------------|-------|
| SRC-001 | Sophos hardens C:\ProgramData\Sophos with explicit non-inheriting DACL; no candidate | (see finding) | (unspecified) | no | — | Auto-enriched from sophos-2026-04-29/001-no-candidate.md. F-001 junction-hunt; closed as no candidate (defense in depth). |
...
|----|-------|---------------|--------|----------|
| [CHAIN-001](#chain-001) | Sophos hardens C:\ProgramData\Sophos with explicit non-inheriting DACL; no candi | `Sophos hardens C:\ProgramData\Sophos with explicit non-inheriting DACL; no candidate` → `Elevated file operation (write / set_acl / read)` | hypothesised | — |

### CHAIN-001 — Sophos hardens C:\ProgramData\Sophos with explicit non-inheriting DACL; no candi

**Status:** ❔ hypothesised  
**CWE:** CWE-269  
**Finding:** [`sophos-2026-04-29/findings/001-no-candidate.md`](../../engagements/sophos-2026-04-29/findings/001-no-candidate.md)  

| Source | Conditions | Sink | Impact |
|--------|------------|------|--------|
| `Sophos hardens C:\ProgramData\Sophos with explicit non-inheriting DACL; no candidate` | (auto-stub: walk the source through any defenses, list each check that must pass) | `Elevated file operation (write / set_acl / read)` | arbitrary file access at privileged principal's level |

```mermaid
flowchart LR
    SRC["Sophos hardens C:\ProgramData\Sophos with explicit non-inheriting DACL; no candidate"]:::src
...
---
_Auto-generated by `scripts/catalog_render.py` at 2026-05-09 15:29 UTC. Edit `catalog/binaries/sophossetup_exe.yml` then re-run the renderer._
