...
```mermaid
flowchart LR
    SRC-001["Multiple process-creation IPC handlers reachable via auth by<br/>I-002 · ac=y · 🟡 partial"]:::grpI
    SRC-002["TeamViewer log files in install dir have Everyone:RW permiss<br/>F-006 · ac=y · 🟡 partial"]:::grpF
    SRC-003["Replace Everyone:RW log file with NTFS hardlink to privilege<br/>F-003 · ac=y · 🟡 partial"]:::grpF
    SRC-004["Updated writeup of IPC auth bypass via unchanged hardcoded M<br/>I-002 · ac=y · 🟡 partial"]:::grpI
    BIN([teamviewer_service.exe<br/>SYSTEM]):::binary
    SNK-001["Elevated file operation &#40;write / set_acl / read&#41;<br/>CWE-269"]:::sink
    SNK-002["IPC handler executes privileged operation<br/>CWE-862"]:::sink
    SRC-001 --> BIN
    SRC-002 --> BIN
    SRC-003 --> BIN
    SRC-004 --> BIN
...
flowchart TB
    subgraph ZSYSTEM["SYSTEM"]
        Nteamviewer_service_exe["**teamviewer_service.exe**<br/>(this binary)"]:::self
    end
...
| `F-006` | Apply protected DACL on file creation; rotate logs to admin-only directories; redact sensitive content before logging. | (audit needed; fill from the finding's defense analysis) | unknown | — |
...
_46 relevant classes; 3 present · 0 defense observed · 0 not present · 43 unchecked_
...
| `F-003` NTFS-hardlink-substitution | 🔴 present | sources: `SRC-003`; chains: `CHAIN-001`, `CHAIN-002`, `CHAIN-003`, `CHAIN-004` |
...
| `F-006` World-writable / user-readable config / log file | 🔴 present | sources: `SRC-002`; chains: `CHAIN-001`, `CHAIN-002`, `CHAIN-003`, `CHAIN-004` |
...
| `I-002` Named-pipe-unauthenticated-write | 🔴 present | sources: `SRC-001`, `SRC-004`; chains: `CHAIN-001`, `CHAIN-002`, `CHAIN-003`, `CHAIN-004` |
...
|----|------|-----|------|---------------------|------------|-------|
| SRC-001 | Multiple process-creation IPC handlers reachable via auth bypass on localhost:5939 | (see finding) | (unspecified) | yes | 15.76.3 | Auto-enriched from teamviewer-2026-03-31/002-ipc-process-creation-surface.md. I-002-style IPC. |
| SRC-002 | TeamViewer log files in install dir have Everyone:RW permissions | (see finding) | (unspecified) | yes | 15.76.3 | Auto-enriched from teamviewer-2026-03-31/004-logfile-weak-permissions.md. F-006 world-writable file/log. |
| SRC-003 | Replace Everyone:RW log file with NTFS hardlink to privileged target | (see finding) | (unspecified) | yes | 15.76.3 | Auto-enriched from teamviewer-2026-03-31/005-arbitrary-system-write-hardlink.md. F-003 NTFS-hardlink-substitution. |
| SRC-004 | Updated writeup of IPC auth bypass via unchanged hardcoded MD5 secret + process spawn | (see finding) | (unspecified) | yes | 15.76.3 | Auto-enriched from teamviewer-2026-03-31/008-ipc-system-process-creation.md. I-002 / localhost IPC; updated. |
...
| SNK-002 | IPC handler executes privileged operation | CWE-862 | (see finding evidence) | arbitrary command execution as listening service | — |
...
|----|-------|---------------|--------|----------|
| [CHAIN-001](#chain-001) | Multiple process-creation IPC handlers reachable via auth bypass on localhost:59 | `Multiple process-creation IPC handlers reachable via auth bypass on localhost:5939` → `IPC handler executes privileged operation` | partial | — |
| [CHAIN-002](#chain-002) | TeamViewer log files in install dir have Everyone:RW permissions | `TeamViewer log files in install dir have Everyone:RW permissions` → `Elevated file operation (write / set_acl / read)` | partial | — |
| [CHAIN-003](#chain-003) | Replace Everyone:RW log file with NTFS hardlink to privileged target | `Replace Everyone:RW log file with NTFS hardlink to privileged target` → `Elevated file operation (write / set_acl / read)` | partial | — |
| [CHAIN-004](#chain-004) | Updated writeup of IPC auth bypass via unchanged hardcoded MD5 secret + process  | `Updated writeup of IPC auth bypass via unchanged hardcoded MD5 secret + process spawn` → `IPC handler executes privileged operation` | partial | — |

### CHAIN-001 — Multiple process-creation IPC handlers reachable via auth bypass on localhost:59

**Status:** 🟡 partial  
**CWE:** CWE-862  
**Finding:** [`teamviewer-2026-03-31/findings/002-ipc-process-creation-surface.md`](../../engagements/teamviewer-2026-03-31/findings/002-ipc-process-creation-surface.md)  

| Source | Conditions | Sink | Impact |
|--------|------------|------|--------|
| `Multiple process-creation IPC handlers reachable via auth bypass on localhost:5939` | (auto-stub: walk the source through any defenses, list each check that must pass) | `IPC handler executes privileged operation` | arbitrary command execution as listening service |

```mermaid
flowchart LR
    SRC["Multiple process-creation IPC handlers reachable via auth bypass on localhost:5939"]:::src
...
### CHAIN-002 — TeamViewer log files in install dir have Everyone:RW permissions
...
| Source | Conditions | Sink | Impact |
|--------|------------|------|--------|
| `TeamViewer log files in install dir have Everyone:RW permissions` | (auto-stub: walk the source through any defenses, list each check that must pass) | `Elevated file operation (write / set_acl / read)` | arbitrary file access at privileged principal's level |

```mermaid
flowchart LR
    SRC["TeamViewer log files in install dir have Everyone:RW permissions"]:::src
    C1["&#40;auto-stub: walk the source through any defenses, list each check that must pass&#41;"]:::cond
    SRC --> C1
...
### CHAIN-003 — Replace Everyone:RW log file with NTFS hardlink to privileged target
...
| Source | Conditions | Sink | Impact |
|--------|------------|------|--------|
| `Replace Everyone:RW log file with NTFS hardlink to privileged target` | (auto-stub: walk the source through any defenses, list each check that must pass) | `Elevated file operation (write / set_acl / read)` | arbitrary file access at privileged principal's level |

```mermaid
flowchart LR
    SRC["Replace Everyone:RW log file with NTFS hardlink to privileged target"]:::src
    C1["&#40;auto-stub: walk the source through any defenses, list each check that must pass&#41;"]:::cond
    SRC --> C1
    SNK["Elevated file operation &#40;write / set_acl / read&#41;"]:::sink
    C1 --> SNK
...
### CHAIN-004 — Updated writeup of IPC auth bypass via unchanged hardcoded MD5 secret + process 
...
| Source | Conditions | Sink | Impact |
|--------|------------|------|--------|
| `Updated writeup of IPC auth bypass via unchanged hardcoded MD5 secret + process spawn` | (auto-stub: walk the source through any defenses, list each check that must pass) | `IPC handler executes privileged operation` | arbitrary command execution as listening service |

```mermaid
flowchart LR
    SRC["Updated writeup of IPC auth bypass via unchanged hardcoded MD5 secret + process spawn"]:::src
    C1["&#40;auto-stub: walk the source through any defenses, list each check that must pass&#41;"]:::cond
    SRC --> C1
    SNK["IPC handler executes privileged operation"]:::sink
    C1 --> SNK
...
---
_Auto-generated by `scripts/catalog_render.py` at 2026-05-09 15:29 UTC. Edit `catalog/binaries/teamviewer_service_exe.yml` then re-run the renderer._
