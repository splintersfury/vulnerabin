...
```mermaid
flowchart LR
    SRC-001["Standard user connects to Everyone-ACLed msgbus pipe; truste<br/>I-002 + T-004 · ac=y · ⏳ unexplored"]:::grpI
    BIN([msgbus.dll<br/>unknown]):::binary
    SNK-001["IPC handler executes privileged operation<br/>CWE-862"]:::sink
    SNK-002["Trust check bypassed; downstream privileged operation accepted<br/>CWE-285"]:::sink
    SRC-001 --> BIN
    BIN --> SNK-001
    BIN --> SNK-002
...
flowchart TB
    ...
    subgraph Zunknown["unknown"]
        Nmsgbus_dll["**msgbus.dll**<br/>(this binary)"]:::self
    end
...
| `T-004` | Don't have IPC at all, OR require fresh authentication beyond image-path identity (HMAC token, mutual TLS, signed session ticket). | (audit needed; fill from the finding's defense analysis) | unknown | — |
...
_41 relevant classes; 2 present · 0 defense observed · 0 not present · 39 unchecked_
...
| `T-004` Caller-process-image-path trust without impersonation | 🔴 present | sources: `SRC-001`; chains: `CHAIN-001` |
...
|----|------|-----|------|---------------------|------------|-------|
| SRC-001 | Standard user connects to Everyone-ACLed msgbus pipe; trusted-process check is FNV-1a hash of caller image path | (see finding) | (unspecified) | yes | — | Auto-enriched from bitdefender-2026-05-02/001-msgbus-trusted-process-bypass.md. Named-pipe write source plus T-004 caller-process-image-path trust check. |
...
| SNK-002 | Trust check bypassed; downstream privileged operation accepted | CWE-285 | (see finding evidence) | auth bypass leads to privileged action | — |
...
|----|-------|---------------|--------|----------|
| [CHAIN-001](#chain-001) | Standard user connects to Everyone-ACLed msgbus pipe; trusted-process check is F | `Standard user connects to Everyone-ACLed msgbus pipe; trusted-process check is FNV-1a hash of caller image path` → `IPC handler executes privileged operation` | unexplored | — |

### CHAIN-001 — Standard user connects to Everyone-ACLed msgbus pipe; trusted-process check is F

**Status:** ⏳ unexplored  
**CWE:** CWE-862  
**Finding:** [`bitdefender-2026-05-02/findings/001-msgbus-trusted-process-bypass.md`](../../engagements/bitdefender-2026-05-02/findings/001-msgbus-trusted-process-bypass.md)  

| Source | Conditions | Sink | Impact |
|--------|------------|------|--------|
| `Standard user connects to Everyone-ACLed msgbus pipe; trusted-process check is FNV-1a hash of caller image path` | (auto-stub: walk the source through any defenses, list each check that must pass) | `IPC handler executes privileged operation` | arbitrary command execution as listening service |

```mermaid
flowchart LR
    SRC["Standard user connects to Everyone-ACLed msgbus pipe; trusted-process check is FNV-1a hash of caller image path"]:::src
...
---
_Auto-generated by `scripts/catalog_render.py` at 2026-05-09 15:29 UTC. Edit `catalog/binaries/msgbus_dll.yml` then re-run the renderer._
