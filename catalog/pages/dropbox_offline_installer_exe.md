...
```mermaid
flowchart LR
    SRC-001["HKLM\Software\DropboxUpdate\...\UninstallCmdLine value &#40;writ<br/>C-001 + UP-005 · ac=y · ⏳ unexplored"]:::grpC
    BIN([dropbox_offline_installer.exe<br/>installer-elevated]):::binary
    SNK-001["Privileged action gated by attacker-writable config<br/>CWE-732"]:::sink
    SNK-002["Updater executes attacker-influenced binary<br/>CWE-494"]:::sink
    SRC-001 --> BIN
    BIN --> SNK-001
    BIN --> SNK-002
    classDef grpC fill:#edc,stroke:#862
...
flowchart TB
    ...
    subgraph Zinstaller_elevated["installer-elevated"]
        Ndropbox_offline_installe["**dropbox_offline_installer.exe**<br/>(this binary)"]:::self
    end
...
|-------|------------------|---------------------|-----|-----------------|
| `C-001` | Admin-only SDDL on the subkey; for HKCU values used by elevated process, validate per-user-context they don't influence cross-user decisions. | (audit needed; fill from the finding's defense analysis) | unknown | — |
| `UP-005` | Extract to admin-write-only path; canonicalize before exec; verify signature on extracted binary before running. | (audit needed; fill from the finding's defense analysis) | unknown | — |
...
_46 relevant classes; 2 present · 0 defense observed · 0 not present · 44 unchecked_
...
| `UP-005` Installer extract-then-execute | 🔴 present | sources: `SRC-001`; chains: `CHAIN-001` |
...
| Class | Status | Rationale / refs |
|-------|--------|-------------------|
| `C-001` Registry value with permissive DACL trusted by elevated | 🔴 present | sources: `SRC-001`; chains: `CHAIN-001` |
...
|----|------|-----|------|---------------------|------------|-------|
| SRC-001 | HKLM\Software\DropboxUpdate\...\UninstallCmdLine value (writable by BUILTIN\Users by design) feeds CreateProcessW from SYSTEM updater | (see finding) | (unspecified) | yes | — | Auto-enriched from dropbox-2026-04-30/003-uninstall-cmd-substring-bypass.md. Mixed: registry-as-source (BUILTIN\Users-writable HKLM key) + updater install class (UP-005). |
...
|----|------|-----|----------|--------|------------|
| SNK-001 | Privileged action gated by attacker-writable config | CWE-732 | (see finding evidence) | privilege transition via config injection | — |
...
|----|-------|---------------|--------|----------|
| [CHAIN-001](#chain-001) | HKLM\Software\DropboxUpdate\...\UninstallCmdLine value (writable by BUILTIN\User | `HKLM\Software\DropboxUpdate\...\UninstallCmdLine value (writable by BUILTIN\Users by design) feeds CreateProcessW from SYSTEM updater` → `Privileged action gated by attacker-writable config` | unexplored | — |

### CHAIN-001 — HKLM\Software\DropboxUpdate\...\UninstallCmdLine value (writable by BUILTIN\User

**Status:** ⏳ unexplored  
**CWE:** CWE-732  
**Finding:** [`dropbox-2026-04-30/findings/003-uninstall-cmd-substring-bypass.md`](../../engagements/dropbox-2026-04-30/findings/003-uninstall-cmd-substring-bypass.md)  

| Source | Conditions | Sink | Impact |
|--------|------------|------|--------|
| `HKLM\Software\DropboxUpdate\...\UninstallCmdLine value (writable by BUILTIN\Users by design) feeds CreateProcessW from SYSTEM updater` | (auto-stub: walk the source through any defenses, list each check that must pass) | `Privileged action gated by attacker-writable config` | privilege transition via config injection |

```mermaid
flowchart LR
    SRC["HKLM\Software\DropboxUpdate\...\UninstallCmdLine value &#40;writable by BUILTIN\Users by design&#41; feeds CreateProcessW from SYSTEM updater"]:::src
    C1["&#40;auto-stub: walk the source through any defenses, list each check that must pass&#41;"]:::cond
    SRC --> C1
    SNK["Privileged action gated by attacker-writable config"]:::sink
    C1 --> SNK
...
---
_Auto-generated by `scripts/catalog_render.py` at 2026-05-09 15:29 UTC. Edit `catalog/binaries/dropbox_offline_installer_exe.yml` then re-run the renderer._
