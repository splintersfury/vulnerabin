...
```mermaid
flowchart LR
    SRC-001["Hardcoded SYSTEM-context paths under user-writable C:\Progra<br/>F-001 · ac=y+caveat · ❔ hypothesised"]:::grpF
    BIN([vantageinstaller.exe<br/>installer-elevated]):::binary
...
flowchart TB
    ...
    subgraph Zinstaller_elevated["installer-elevated"]
        Nvantageinstaller_exe["**vantageinstaller.exe**<br/>(this binary)"]:::self
    end
...
|-------|--------|-------------------|
| `F-001` NTFS-junction-write-by-elevated-process | 🔴 present | sources: `SRC-001`; chains: `CHAIN-001` |
...
|---------|------------|------------|--------|-------|
| — | 2026-04-29 | lenovo-vantage-2026-04-29 | — | — |
...
|----|------|-----|------|---------------------|------------|-------|
| SRC-001 | Hardcoded SYSTEM-context paths under user-writable C:\ProgramData\Lenovo\Vantage; install rejects test flags | (see finding) | (unspecified) | yes (caveat: Static lead; could not complete dynamic install (enterprise-deployment config)) | — | Auto-enriched from lenovo-vantage-2026-04-29/001-junction-candidates.md. F-001 junction-hunt; static lead. |
...
|----|-------|---------------|--------|----------|
| [CHAIN-001](#chain-001) | Hardcoded SYSTEM-context paths under user-writable C:\ProgramData\Lenovo\Vantage | `Hardcoded SYSTEM-context paths under user-writable C:\ProgramData\Lenovo\Vantage; install rejects test flags` → `Elevated file operation (write / set_acl / read)` | hypothesised | — |

### CHAIN-001 — Hardcoded SYSTEM-context paths under user-writable C:\ProgramData\Lenovo\Vantage

**Status:** ❔ hypothesised  
**CWE:** CWE-269  
**Finding:** [`lenovo-vantage-2026-04-29/findings/001-junction-candidates.md`](../../engagements/lenovo-vantage-2026-04-29/findings/001-junction-candidates.md)  

| Source | Conditions | Sink | Impact |
|--------|------------|------|--------|
| `Hardcoded SYSTEM-context paths under user-writable C:\ProgramData\Lenovo\Vantage; install rejects test flags` | (auto-stub: walk the source through any defenses, list each check that must pass) | `Elevated file operation (write / set_acl / read)` | arbitrary file access at privileged principal's level |

```mermaid
flowchart LR
    SRC["Hardcoded SYSTEM-context paths under user-writable C:\ProgramData\Lenovo\Vantage; install rejects test flags"]:::src
...
---
_Auto-generated by `scripts/catalog_render.py` at 2026-05-09 15:29 UTC. Edit `catalog/binaries/vantageinstaller_exe.yml` then re-run the renderer._
