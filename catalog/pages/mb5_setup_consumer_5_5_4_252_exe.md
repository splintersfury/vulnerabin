# mb5-setup-consumer-5.5.4.252.exe

(Auto-seeded; replace this description.) Catalogued in 1 engagement(s).

## At a glance

- **Binary**: `mb5-setup-consumer-5.5.4.252.exe`
- **Binary Kind**: exe
...
```mermaid
flowchart LR
    SRC-001["Original junction-write claim invalidated by live access-den<br/>F-001 · ac=n · ❔ hypothesised"]:::grpF
    SRC-002["Original ICACLS-based junction-prerequisite claim &#40;later inv<br/>F-001 · ac=y · ❔ hypothesised"]:::grpF
    BIN([mb5-setup-consumer-5.5.4.252.exe<br/>installer-elevated]):::binary
...
flowchart TB
    ...
    subgraph Zinstaller_elevated["installer-elevated"]
        Nmb5_setup_consumer_5_5_4["**mb5-setup-consumer-5.5.4.252.exe**<br/>(this binary)"]:::self
    end
...
|-------|------------------|---------------------|-----|-----------------|
| `F-001` | Apply protected DACL to every ancestor the binary creates; open paths with FILE_OPEN_REPARSE_POINT/OBJ_DONT_REPARSE; refuse if FILE_ATTRIBUTE_REPARSE_POINT is set on any walked component; OR impersona | (audit needed; fill from the finding's defense analysis) | unknown | — |
...
_46 relevant classes; 1 present · 0 defense observed · 0 not present · 45 unchecked_

### Group F

| Class | Status | Rationale / refs |
|-------|--------|-------------------|
| `F-001` NTFS-junction-write-by-elevated-process | 🔴 present | sources: `SRC-001`, `SRC-002`; chains: `CHAIN-001`, `CHAIN-002` |
...
|---------|------------|------------|--------|-------|
| — | 2026-04-29 | malwarebytes-2026-04-29 | — | — |
...
|----|------|-----|------|---------------------|------------|-------|
| SRC-001 | Original junction-write claim invalidated by live access-denied on every standard-user write attempt | (see finding) | (unspecified) | no | — | Auto-enriched from malwarebytes-2026-04-29/001-junction-prerequisite-mbamservice-INVALIDATED.md. F-001 junction-hunt; INVALIDATED. |
| SRC-002 | Original ICACLS-based junction-prerequisite claim (later invalidated) | (see finding) | (unspecified) | yes | — | Auto-enriched from malwarebytes-2026-04-29/001-junction-prerequisite-mbamservice.ORIGINAL.md. Original version; later invalidated. Should arguably skip per WIP rule but kept for completeness. |
...
|----|-------|---------------|--------|----------|
| [CHAIN-001](#chain-001) | Original junction-write claim invalidated by live access-denied on every standar | `Original junction-write claim invalidated by live access-denied on every standard-user write attempt` → `Elevated file operation (write / set_acl / read)` | hypothesised | — |
| [CHAIN-002](#chain-002) | Original ICACLS-based junction-prerequisite claim (later invalidated) | `Original ICACLS-based junction-prerequisite claim (later invalidated)` → `Elevated file operation (write / set_acl / read)` | hypothesised | — |

### CHAIN-001 — Original junction-write claim invalidated by live access-denied on every standar

**Status:** ❔ hypothesised  
**CWE:** CWE-269  
**Finding:** [`malwarebytes-2026-04-29/findings/001-junction-prerequisite-mbamservice-INVALIDATED.md`](../../engagements/malwarebytes-2026-04-29/findings/001-junction-prerequisite-mbamservice-INVALIDATED.md)  

| Source | Conditions | Sink | Impact |
|--------|------------|------|--------|
| `Original junction-write claim invalidated by live access-denied on every standard-user write attempt` | (auto-stub: walk the source through any defenses, list each check that must pass) | `Elevated file operation (write / set_acl / read)` | arbitrary file access at privileged principal's level |

```mermaid
flowchart LR
    SRC["Original junction-write claim invalidated by live access-denied on every standard-user write attempt"]:::src
...
### CHAIN-002 — Original ICACLS-based junction-prerequisite claim (later invalidated)
...
| Source | Conditions | Sink | Impact |
|--------|------------|------|--------|
| `Original ICACLS-based junction-prerequisite claim (later invalidated)` | (auto-stub: walk the source through any defenses, list each check that must pass) | `Elevated file operation (write / set_acl / read)` | arbitrary file access at privileged principal's level |

```mermaid
flowchart LR
    SRC["Original ICACLS-based junction-prerequisite claim &#40;later invalidated&#41;"]:::src
    C1["&#40;auto-stub: walk the source through any defenses, list each check that must pass&#41;"]:::cond
    SRC --> C1
    SNK["Elevated file operation &#40;write / set_acl / read&#41;"]:::sink
    C1 --> SNK
...
---
_Auto-generated by `scripts/catalog_render.py` at 2026-05-09 15:29 UTC. Edit `catalog/binaries/mb5_setup_consumer_5_5_4_252_exe.yml` then re-run the renderer._
