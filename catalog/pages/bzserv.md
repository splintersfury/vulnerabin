...
```mermaid
flowchart LR
    SRC-001["Standard user replaces ProgramData log file with NTFS hardli<br/>F-003 · ac=y · ⏳ unexplored"]:::grpF
    SRC-002["Auto-updater downloads and executes binary without Authentic<br/>UP-001 · ac=y · ⏳ unexplored"]:::grpUP
    BIN([bzserv<br/>unknown]):::binary
    SNK-001["Elevated file operation &#40;write / set_acl / read&#41;<br/>CWE-269"]:::sink
    SNK-002["Updater executes attacker-influenced binary<br/>CWE-494"]:::sink
    SRC-001 --> BIN
    SRC-002 --> BIN
    BIN --> SNK-001
    BIN --> SNK-002
    classDef grpF fill:#fdd,stroke:#c00
    classDef grpUP fill:#ffd,stroke:#aa0
...
flowchart TB
    ...
    subgraph Zunknown["unknown"]
        Nbzserv["**bzserv**<br/>(this binary)"]:::self
    end
...
|-------|------------------|---------------------|-----|-----------------|
| `F-003` | Open with FILE_FLAG_OPEN_REPARSE_POINT, verify nNumberOfLinks == 1 via GetFileInformationByHandle; or use FILE_DISPOSITION_POSIX_SEMANTICS for delete operations. | (audit needed; fill from the finding's defense analysis) | unknown | — |
| `UP-001` | WinVerifyTrust on every downloaded binary before exec; signature pinning with backup keys; refuse exec on signature failure. | (audit needed; fill from the finding's defense analysis) | unknown | — |
...
_62 relevant classes; 2 present · 0 defense observed · 0 not present · 60 unchecked_
...
| `F-003` NTFS-hardlink-substitution | 🔴 present | sources: `SRC-001`; chains: `CHAIN-001`, `CHAIN-002` |
...
| Class | Status | Rationale / refs |
|-------|--------|-------------------|
| `UP-001` Auto-updater unsigned-binary execution | 🔴 present | sources: `SRC-002`; chains: `CHAIN-001`, `CHAIN-002` |
...
| `E-006` Renderer XSS in trusted origin | ⏳ unchecked | _walk the detection checklist below_ |
...
<details><summary>Detection checklist for <code>E-006</code> — Renderer XSS in trusted origin</summary>

**Canonical defense:** DOMPurify on user-rendered content; CSP with strict-dynamic; nonce-based script loading.

**Detection checklist:**
- [ ] Q1: does the renderer render user-supplied content (messages, names, URLs) into HTML?
- [ ] Q2: is innerHTML / dangerouslySetInnerHTML / Vue v-html / Angular [innerHTML] used?
- [ ] Q3: is sanitization absent or bypassable?
- [ ] Q4: does the page have contextBridge-exposed APIs that the XSS can call?
- [ ] Q5: does the page run with privileged origin?
- [ ] 5/5 → present.
...
|---------|------------|------------|--------|-------|
| — | 2026-04-08 | backblaze-2026-04-08 | — | Auto-seeded from backblaze-2026-04-08/scope.json (target: Backblaze Personal Backup) |
...
|----|------|-----|------|---------------------|------------|-------|
| SRC-001 | Standard user replaces ProgramData log file with NTFS hardlink to privileged target; SYSTEM service writes through | (see finding) | (unspecified) | yes | — | Auto-enriched from backblaze-2026-04-08/001-log-file-hardlink-lpe.md. NTFS hardlink (F-003) per draft taxonomy. Bugcrowd state: marked Duplicate $0. |
| SRC-002 | Auto-updater downloads and executes binary without Authenticode signature verification | (see finding) | (unspecified) | yes | — | Auto-enriched from backblaze-2026-04-08/002-unsigned-update-execution-lpe.md. Textbook UP-001 unsigned-update execution. |
...
|----|------|-----|----------|--------|------------|
| SNK-001 | Elevated file operation (write / set_acl / read) | CWE-269 | (see finding evidence) | arbitrary file access at privileged principal's level | — |
| SNK-002 | Updater executes attacker-influenced binary | CWE-494 | (see finding evidence) | code execution at updater's privilege | — |
...
|----|-------|---------------|--------|----------|
| [CHAIN-001](#chain-001) | Standard user replaces ProgramData log file with NTFS hardlink to privileged tar | `Standard user replaces ProgramData log file with NTFS hardlink to privileged target; SYSTEM service writes through` → `Elevated file operation (write / set_acl / read)` | mitigated | — |
| [CHAIN-002](#chain-002) | Auto-updater downloads and executes binary without Authenticode signature verifi | `Auto-updater downloads and executes binary without Authenticode signature verification` → `Updater executes attacker-influenced binary` | unexplored | — |

### CHAIN-001 — Standard user replaces ProgramData log file with NTFS hardlink to privileged tar

**Status:** 🛡 mitigated  
**CWE:** CWE-269  
**Finding:** [`backblaze-2026-04-08/findings/001-log-file-hardlink-lpe.md`](../../engagements/backblaze-2026-04-08/findings/001-log-file-hardlink-lpe.md)  

| Source | Conditions | Sink | Impact |
|--------|------------|------|--------|
| `Standard user replaces ProgramData log file with NTFS hardlink to privileged target; SYSTEM service writes through` | (auto-stub: walk the source through any defenses, list each check that must pass) | `Elevated file operation (write / set_acl / read)` | arbitrary file access at privileged principal's level |

```mermaid
flowchart LR
    SRC["Standard user replaces ProgramData log file with NTFS hardlink to privileged target; SYSTEM service writes through"]:::src
    C1["&#40;auto-stub: walk the source through any defenses, list each check that must pass&#41;"]:::cond
    SRC --> C1
    SNK["Elevated file operation &#40;write / set_acl / read&#41;"]:::sink
    C1 --> SNK
...
### CHAIN-002 — Auto-updater downloads and executes binary without Authenticode signature verifi
...
| Source | Conditions | Sink | Impact |
|--------|------------|------|--------|
| `Auto-updater downloads and executes binary without Authenticode signature verification` | (auto-stub: walk the source through any defenses, list each check that must pass) | `Updater executes attacker-influenced binary` | code execution at updater's privilege |

```mermaid
flowchart LR
    SRC["Auto-updater downloads and executes binary without Authenticode signature verification"]:::src
    C1["&#40;auto-stub: walk the source through any defenses, list each check that must pass&#41;"]:::cond
    SRC --> C1
    SNK["Updater executes attacker-influenced binary"]:::sink
    C1 --> SNK
    classDef src fill:#dff,stroke:#069
    classDef cond fill:#ffd,stroke:#960
    classDef sink fill:#fdd,stroke:#900
...
---
_Auto-generated by `scripts/catalog_render.py` at 2026-05-09 15:29 UTC. Edit `catalog/binaries/bzserv.yml` then re-run the renderer._
