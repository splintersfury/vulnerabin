...
```mermaid
flowchart LR
    SRC-001["User-writable C:\ProgramData\HP\* dirs accept junctions; nee<br/>F-001 · ac=y+caveat · ❔ hypothesised"]:::grpF
    BIN([hp_support_assistant.exe<br/>unknown]):::binary
...
flowchart TB
    ...
    subgraph Zunknown["unknown"]
        Nhp_support_assistant_exe["**hp_support_assistant.exe**<br/>(this binary)"]:::self
    end
...
|---------|------------|------------|--------|-------|
| — | 2026-04-29 | hp-support-assistant-2026-04-29 | — | — |
...
|----|------|-----|------|---------------------|------------|-------|
| SRC-001 | User-writable C:\ProgramData\HP\* dirs accept junctions; needs HPSA UWP trigger | (see finding) | (unspecified) | yes (caveat: Junction primitive works but practical trigger requires HPSA UWP appxbundle (not on test bench)) | — | Auto-enriched from hp-support-assistant-2026-04-29/001-junction-candidates.md. F-001 junction-write hunt; closed as needs-trigger. |
...
|----|-------|---------------|--------|----------|
| [CHAIN-001](#chain-001) | User-writable C:\ProgramData\HP\* dirs accept junctions; needs HPSA UWP trigger | `User-writable C:\ProgramData\HP\* dirs accept junctions; needs HPSA UWP trigger` → `Elevated file operation (write / set_acl / read)` | hypothesised | — |

### CHAIN-001 — User-writable C:\ProgramData\HP\* dirs accept junctions; needs HPSA UWP trigger

**Status:** ❔ hypothesised  
**CWE:** CWE-269  
**Finding:** [`hp-support-assistant-2026-04-29/findings/001-junction-candidates.md`](../../engagements/hp-support-assistant-2026-04-29/findings/001-junction-candidates.md)  

| Source | Conditions | Sink | Impact |
|--------|------------|------|--------|
| `User-writable C:\ProgramData\HP\* dirs accept junctions; needs HPSA UWP trigger` | (auto-stub: walk the source through any defenses, list each check that must pass) | `Elevated file operation (write / set_acl / read)` | arbitrary file access at privileged principal's level |

```mermaid
flowchart LR
    SRC["User-writable C:\ProgramData\HP\* dirs accept junctions; needs HPSA UWP trigger"]:::src
...
---
_Auto-generated by `scripts/catalog_render.py` at 2026-05-09 15:29 UTC. Edit `catalog/binaries/hp_support_assistant_exe.yml` then re-run the renderer._
