...
```mermaid
flowchart LR
    SRC-001["Everyone-ACL named pipes accept process-spawn task GUIDs to <br/>I-002 · ac=y · ⏳ unexplored"]:::grpI
    BIN([broker<br/>SYSTEM]):::binary
    SNK-001["IPC handler executes privileged operation<br/>CWE-862"]:::sink
    SRC-001 --> BIN
    BIN --> SNK-001
...
flowchart TB
    subgraph ZSYSTEM["SYSTEM"]
        Nbroker["**broker**<br/>(this binary)"]:::self
    end
...
|-------|------------------|---------------------|-----|-----------------|
| `I-002` | Explicit SDDL; authenticate caller via ImpersonateNamedPipeClient + group membership check, OR GetNamedPipeClientProcessId + image-path allowlist. | (audit needed; fill from the finding's defense analysis) | unknown | — |
...
| `I-002` Named-pipe-unauthenticated-write | 🔴 present | sources: `SRC-001`; chains: `CHAIN-001` |
...
|----|------|-----|------|---------------------|------------|-------|
| SRC-001 | Everyone-ACL named pipes accept process-spawn task GUIDs to SYSTEM broker | (see finding) | (unspecified) | yes | 27.x | Auto-enriched from bitdefender-total-security-2026-04-11/002-process-broker-pipe-lpe.md. I-002 named-pipe-unauth-write. |
...
|----|------|-----|----------|--------|------------|
| SNK-001 | IPC handler executes privileged operation | CWE-862 | (see finding evidence) | arbitrary command execution as listening service | — |
...
|----|-------|---------------|--------|----------|
| [CHAIN-001](#chain-001) | Everyone-ACL named pipes accept process-spawn task GUIDs to SYSTEM broker | `Everyone-ACL named pipes accept process-spawn task GUIDs to SYSTEM broker` → `IPC handler executes privileged operation` | unexplored | — |

### CHAIN-001 — Everyone-ACL named pipes accept process-spawn task GUIDs to SYSTEM broker

**Status:** ⏳ unexplored  
**CWE:** CWE-862  
**Finding:** [`bitdefender-total-security-2026-04-11/findings/002-process-broker-pipe-lpe.md`](../../engagements/bitdefender-total-security-2026-04-11/findings/002-process-broker-pipe-lpe.md)  

| Source | Conditions | Sink | Impact |
|--------|------------|------|--------|
| `Everyone-ACL named pipes accept process-spawn task GUIDs to SYSTEM broker` | (auto-stub: walk the source through any defenses, list each check that must pass) | `IPC handler executes privileged operation` | arbitrary command execution as listening service |

```mermaid
flowchart LR
    SRC["Everyone-ACL named pipes accept process-spawn task GUIDs to SYSTEM broker"]:::src
    C1["&#40;auto-stub: walk the source through any defenses, list each check that must pass&#41;"]:::cond
    SRC --> C1
    SNK["IPC handler executes privileged operation"]:::sink
    C1 --> SNK
...
---
_Auto-generated by `scripts/catalog_render.py` at 2026-05-09 15:29 UTC. Edit `catalog/binaries/broker.yml` then re-run the renderer._
