...
```mermaid
flowchart LR
    SRC-001["Server-supplied TL-deserialized photo metadata triggers OOB <br/>N-003 + N-004 · ac=y+caveat · ⏳ unexplored"]:::grpN
    SRC-002["Crafted TGS sticker &#40;gzip+JSON&#41; sent in chat; recursive prec<br/>N-004 · ac=y · ⏳ unexplored"]:::grpN
    SRC-003["tg://bg URL routing has stray ^ in regex; cosmetic only<br/>U-003 · ac=y · ❔ hypothesised"]:::grpU
    SRC-004["IV renderer puts URL into <iframe srcdoc> instead of src; la<br/>E-006 · ac=y+caveat · ❔ hypothesised"]:::grpE
    BIN([telegram<br/>unknown]):::binary
    SNK-001["Renderer-context API call reaches main process<br/>CWE-94"]:::sink
    SNK-002["Network parser memory corruption / logic flaw<br/>CWE-787"]:::sink
    SNK-003["Privileged operation driven by user-input<br/>CWE-269"]:::sink
    SRC-001 --> BIN
    SRC-002 --> BIN
    SRC-003 --> BIN
    SRC-004 --> BIN
    ...
    BIN --> SNK-002
    BIN --> SNK-003
    classDef grpE fill:#bdf,stroke:#069
    classDef grpN fill:#ddf,stroke:#06c
    classDef grpU fill:#dfd,stroke:#080
...
flowchart TB
    ...
    subgraph Zunknown["unknown"]
        Ntelegram["**telegram**<br/>(this binary)"]:::self
    end
...
|-------|------------------|---------------------|-----|-----------------|
| `E-006` | DOMPurify on user-rendered content; CSP with strict-dynamic; nonce-based script loading. | (audit needed; fill from the finding's defense analysis) | unknown | — |
| `N-003` | Cert pinning; HMAC over payloads; protocol parser fuzzing. | (audit needed; fill from the finding's defense analysis) | unknown | — |
| `N-004` | Use schema-driven parsers (protobuf, capnproto); explicit type tags; bounds-checked TLV walks. | (audit needed; fill from the finding's defense analysis) | unknown | — |
| `U-003` | Strict URL parser; reject unexpected URL components; treat URL as untrusted input regardless of source. | (audit needed; fill from the finding's defense analysis) | unknown | — |
...
_62 relevant classes; 4 present · 0 defense observed · 0 not present · 58 unchecked_
...
| `N-003` TLS / network protocol parser (transport-level) | 🔴 present | sources: `SRC-001`; chains: `CHAIN-001`, `CHAIN-002`, `CHAIN-003`, `CHAIN-004` |
| `N-004` Custom application protocol (payload-level) | 🔴 present | sources: `SRC-001`, `SRC-002`; chains: `CHAIN-001`, `CHAIN-002`, `CHAIN-003`, `CHAIN-004` |
...
| `U-003` Custom-protocol-handler URL routing | 🔴 present | sources: `SRC-003`; chains: `CHAIN-001`, `CHAIN-002`, `CHAIN-003`, `CHAIN-004` |
...
| Class | Status | Rationale / refs |
|-------|--------|-------------------|
| `E-001` ipcMain.on/handle channel | ⏳ unchecked | _walk the detection checklist below_ |
| `E-002` Custom protocol handler URL routing (Electron) | ⏳ unchecked | _walk the detection checklist below_ |
| `E-003` webPreferences.nodeIntegration:true | ⏳ unchecked | _walk the detection checklist below_ |
| `E-004` protocol.registerFileProtocol unvalidated | ⏳ unchecked | _walk the detection checklist below_ |
| `E-005` BrowserWindow.loadURL with attacker URL | ⏳ unchecked | _walk the detection checklist below_ |
| `E-006` Renderer XSS in trusted origin | 🔴 present | sources: `SRC-004`; chains: `CHAIN-001`, `CHAIN-002`, `CHAIN-003`, `CHAIN-004` |
| `E-007` Squirrel/electron-updater feed (Electron-specific overlap UP | ⏳ unchecked | _walk the detection checklist below_ |

<details><summary>Detection checklist for <code>E-001</code> — ipcMain.on/handle channel</summary>

**Canonical defense:** contextIsolation: true; validate every handler argument; never pass renderer-supplied args directly to child_process.exec / fs.writeFile / eval; use a strict schema validator (zod, joi) per channel.

**Common bypasses:**
- Renderer XSS reaches handler with attacker-controlled args
- contextBridge exposes raw fs API

**Detection checklist:**
- [ ] Q1: does the app register ipcMain.on('channel', handler) or ipcMain.handle?
- [ ] Q2: do handler signatures take path / command / URL arguments?
- [ ] Q3: are arguments passed to fs.* / child_process.* / require() without validation?
- [ ] Q4: is contextIsolation: false anywhere?
- [ ] Q5: are renderer pages loading untrusted content?
- [ ] 5/5 → present.

</details>

<details><summary>Detection checklist for <code>E-002</code> — Custom protocol handler URL routing (Electron)</summary>

**Canonical defense:** Strict URL parser; reject unexpected paths; treat URL as untrusted input.

**Detection checklist:**
- [ ] Q1: does the app call app.setAsDefaultProtocolClient?
- [ ] Q2: is URL parsing in second-instance / open-url handler hand-rolled?
- [ ] Q3: do URL fragments become file paths / IPC channels?
- [ ] Q4: are downstream operations privileged?
- [ ] Q5: is no signature/auth check on URL?
- [ ] 5/5 → present.

</details>

<details><summary>Detection checklist for <code>E-003</code> — webPreferences.nodeIntegration:true</summary>

**Canonical defense:** contextIsolation: true; nodeIntegration: false (default modern Electron); use contextBridge.exposeInMainWorld for explicit API surface.

**Detection checklist:**
- [ ] Q1: does any BrowserWindow have webPreferences.nodeIntegration: true?
- [ ] Q2: is contextIsolation: false anywhere?
- [ ] Q3: is webSecurity: false anywhere?
- [ ] Q4: is the renderer loading any untrusted (cross-origin) content?
- [ ] Q5: is there XSS in any page loaded by the renderer?
- [ ] 5/5 → present (any single yes is concerning).

</details>

<details><summary>Detection checklist for <code>E-004</code> — protocol.registerFileProtocol unvalidated</summary>

**Canonical defense:** Strict URL → path mapping; reject paths with .. or absolute paths; canonicalize and prefix-check against an expected base.

**Detection checklist:**
- [ ] Q1: does the app call protocol.registerFileProtocol or protocol.registerStreamProtocol?
- [ ] Q2: is the URL → path resolver hand-rolled?
- [ ] Q3: does it accept user-controlled URL fragments?
- [ ] Q4: is canonicalization absent?
- [ ] Q5: does the resolved path get read / loaded as a script?
- [ ] 5/5 → present.

</details>

<details><summary>Detection checklist for <code>E-005</code> — BrowserWindow.loadURL with attacker URL</summary>

**Canonical defense:** Allowlist origins for loadURL; strip query/fragment from user-supplied URLs; never disable webSecurity.

**Detection checklist:**
- [ ] Q1: does the app call loadURL with a URL derived from user input?
- [ ] Q2: is the origin allowlist absent?
- [ ] Q3: is webSecurity: false?
- [ ] Q4: can user-controlled origin become loaded?
- [ ] Q5: does the BrowserWindow have privileged preload exposing APIs?
- [ ] 5/5 → present.
...
| Class | Status | Rationale / refs |
|-------|--------|-------------------|
| `W-001` Authenticated user session as source | ⏳ unchecked | _walk the detection checklist below_ |
| `W-002` Cross-tenant authorization gap | ⏳ unchecked | _walk the detection checklist below_ |
| `W-003` Pre-auth endpoint | ⏳ unchecked | _walk the detection checklist below_ |

<details><summary>Detection checklist for <code>W-001</code> — Authenticated user session as source</summary>

**Canonical defense:** Tenant-scope every request; explicit authorization check on every endpoint; per-row access control.

**Detection checklist:**
- [ ] Q1: does the API endpoint take an object ID as parameter?
- [ ] Q2: is per-tenant / per-user filtering missing?
- [ ] Q3: are object IDs sequential / predictable?
- [ ] Q4: does the endpoint return data for objects the requester doesn't own?
- [ ] Q5: is the response schema not filtered server-side?
- [ ] 5/5 → present.

</details>

<details><summary>Detection checklist for <code>W-002</code> — Cross-tenant authorization gap</summary>

**Canonical defense:** Tenant ID required on every query; SQL/ORM scope filters set by middleware not handler.

**Detection checklist:**
- [ ] Subset of W-001 — see that checklist.

</details>

<details><summary>Detection checklist for <code>W-003</code> — Pre-auth endpoint</summary>

**Canonical defense:** Auth required by default; explicit allowlist for genuinely public endpoints; rate-limit.

**Detection checklist:**
- [ ] Q1: does any endpoint accept requests with no authentication?
- [ ] Q2: does it return / mutate sensitive data?
- [ ] Q3: is rate-limiting absent?
- [ ] Q4: is input validation lax?
- [ ] Q5: does the endpoint trigger privileged backend operations?
- [ ] 5/5 → present.
...
|---------|------------|------------|--------|-------|
| — | 2026-03-29 | telegram-desktop-2026-03-29 | — | — |
...
|----|------|-----|------|---------------------|------------|-------|
| SRC-001 | Server-supplied TL-deserialized photo metadata triggers OOB read | (see finding) | (unspecified) | yes (caveat: Requires malicious Telegram server, MITM on MTProto, or server-side vuln) | — | Auto-enriched from telegram-desktop-2026-03-29/001-mtpphoto-oob-read.md. Custom MTProto protocol parser; N-004 is closer fit. Server-as-attacker. |
| SRC-002 | Crafted TGS sticker (gzip+JSON) sent in chat; recursive precomp parsing exhausts stack | (see finding) | (unspecified) | yes | — | Auto-enriched from telegram-desktop-2026-03-29/002-lottie-no-recursion-limit.md. Application-level message parser; N-004 custom application protocol. |
| SRC-003 | tg://bg URL routing has stray ^ in regex; cosmetic only | (see finding) | (unspecified) | yes | — | Auto-enriched from telegram-desktop-2026-03-29/003-bg-regex-typo.md. U-003 custom-protocol-handler URL routing; closed as not security. |
| SRC-004 | IV renderer puts URL into <iframe srcdoc> instead of src; latent XSS class | (see finding) | (unspecified) | yes (caveat: Weaponization gated on server validation of URL field) | — | Auto-enriched from telegram-desktop-2026-03-29/004-iv-srcdoc-url-injection.md. Calibration target: E-006 renderer XSS in trusted origin. |
...
|----|------|-----|----------|--------|------------|
| SNK-001 | Renderer-context API call reaches main process | CWE-94 | (see finding evidence) | code execution in main process | — |
| SNK-002 | Network parser memory corruption / logic flaw | CWE-787 | (see finding evidence) | remote code execution or DoS | — |
| SNK-003 | Privileged operation driven by user-input | CWE-269 | (see finding evidence) | privilege transition via crafted input | — |
...
|----|-------|---------------|--------|----------|
| [CHAIN-001](#chain-001) | Server-supplied TL-deserialized photo metadata triggers OOB read | `Server-supplied TL-deserialized photo metadata triggers OOB read` → `Network parser memory corruption / logic flaw` | unexplored | — |
| [CHAIN-002](#chain-002) | Crafted TGS sticker (gzip+JSON) sent in chat; recursive precomp parsing exhausts | `Crafted TGS sticker (gzip+JSON) sent in chat; recursive precomp parsing exhausts stack` → `Network parser memory corruption / logic flaw` | unexplored | — |
| [CHAIN-003](#chain-003) | tg://bg URL routing has stray ^ in regex; cosmetic only | `tg://bg URL routing has stray ^ in regex; cosmetic only` → `Privileged operation driven by user-input` | hypothesised | — |
| [CHAIN-004](#chain-004) | IV renderer puts URL into <iframe srcdoc> instead of src; latent XSS class | `IV renderer puts URL into <iframe srcdoc> instead of src; latent XSS class` → `Renderer-context API call reaches main process` | hypothesised | — |

### CHAIN-001 — Server-supplied TL-deserialized photo metadata triggers OOB read

**Status:** ⏳ unexplored  
**CWE:** CWE-787  
**Finding:** [`telegram-desktop-2026-03-29/findings/001-mtpphoto-oob-read.md`](../../engagements/telegram-desktop-2026-03-29/findings/001-mtpphoto-oob-read.md)  

| Source | Conditions | Sink | Impact |
|--------|------------|------|--------|
| `Server-supplied TL-deserialized photo metadata triggers OOB read` | (auto-stub: walk the source through any defenses, list each check that must pass) | `Network parser memory corruption / logic flaw` | remote code execution or DoS |

```mermaid
flowchart LR
    SRC["Server-supplied TL-deserialized photo metadata triggers OOB read"]:::src
    C1["&#40;auto-stub: walk the source through any defenses, list each check that must pass&#41;"]:::cond
    SRC --> C1
    SNK["Network parser memory corruption / logic flaw"]:::sink
    C1 --> SNK
...
### CHAIN-002 — Crafted TGS sticker (gzip+JSON) sent in chat; recursive precomp parsing exhausts
...
| Source | Conditions | Sink | Impact |
|--------|------------|------|--------|
| `Crafted TGS sticker (gzip+JSON) sent in chat; recursive precomp parsing exhausts stack` | (auto-stub: walk the source through any defenses, list each check that must pass) | `Network parser memory corruption / logic flaw` | remote code execution or DoS |

```mermaid
flowchart LR
    SRC["Crafted TGS sticker &#40;gzip+JSON&#41; sent in chat; recursive precomp parsing exhausts stack"]:::src
    C1["&#40;auto-stub: walk the source through any defenses, list each check that must pass&#41;"]:::cond
    SRC --> C1
    SNK["Network parser memory corruption / logic flaw"]:::sink
    C1 --> SNK
...
### CHAIN-003 — tg://bg URL routing has stray ^ in regex; cosmetic only
...
| Source | Conditions | Sink | Impact |
|--------|------------|------|--------|
| `tg://bg URL routing has stray ^ in regex; cosmetic only` | (auto-stub: walk the source through any defenses, list each check that must pass) | `Privileged operation driven by user-input` | privilege transition via crafted input |

```mermaid
flowchart LR
    SRC["tg://bg URL routing has stray ^ in regex; cosmetic only"]:::src
    C1["&#40;auto-stub: walk the source through any defenses, list each check that must pass&#41;"]:::cond
    SRC --> C1
    SNK["Privileged operation driven by user-input"]:::sink
    C1 --> SNK
    classDef src fill:#dff,stroke:#069
    classDef cond fill:#ffd,stroke:#960
    classDef sink fill:#fdd,stroke:#900
...
### CHAIN-004 — IV renderer puts URL into <iframe srcdoc> instead of src; latent XSS class
...
| Source | Conditions | Sink | Impact |
|--------|------------|------|--------|
| `IV renderer puts URL into <iframe srcdoc> instead of src; latent XSS class` | (auto-stub: walk the source through any defenses, list each check that must pass) | `Renderer-context API call reaches main process` | code execution in main process |

```mermaid
flowchart LR
    SRC["IV renderer puts URL into <iframe srcdoc> instead of src; latent XSS class"]:::src
    C1["&#40;auto-stub: walk the source through any defenses, list each check that must pass&#41;"]:::cond
    SRC --> C1
    SNK["Renderer-context API call reaches main process"]:::sink
    C1 --> SNK
    classDef src fill:#dff,stroke:#069
    classDef cond fill:#ffd,stroke:#960
    classDef sink fill:#fdd,stroke:#900
...
---
_Auto-generated by `scripts/catalog_render.py` at 2026-05-09 15:29 UTC. Edit `catalog/binaries/telegram.yml` then re-run the renderer._
